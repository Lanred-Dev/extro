# Honestly I hope I never have to look at this code again, to much math
...
cdef Vector2 project_polygon(Vector2 axis, list[Vector2] vertices):
    ...
    for index in range(length):
        dots[index] = vertices[index].dot(axis)
...
cdef class CollisionMask:
    ...
    cpdef object collides_with(self, CollisionMask other_collision_mask):
        self._recompute_if_dirty()
        other_collision_mask._recompute_if_dirty()
        ...
        cdef float overlap
        cdef float min_overlap = 1e9
        cdef Vector2 smallest_axis = Vector2(0.0, 0.0)
        ...
        for index in range(len(axes)):
            ...
            # calculate overlap
            overlap = min(projection1.y, projection2.y) - max(projection1.x, projection2.x)

            if overlap < min_overlap:
                min_overlap = overlap
                smallest_axis = axis

        # ensure axis points from self -> other
        cdef Vector2 d = Vector2(other_collision_mask.position.x - self.position.x,
                                other_collision_mask.position.y - self.position.y)

        if d.__cdot__(smallest_axis) < 0:
            smallest_axis = Vector2(-smallest_axis.x, -smallest_axis.y)
