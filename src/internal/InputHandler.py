...
class InputHandlerCls:
    ...
    def __init__(self):
        ...
        Window.push_handlers(
            ...
            on_key_release=self._handle_key_release,
            on_mouse_motion=self._handle_mouse_move,
            on_mouse_press=self._handle_mouse_click,
        )

    def _handle_key_press(self, key: int, modifiers: List[int]):
        self.active_keys.append(key)
        self.on_key_press.fire(key, modifiers)

    def _handle_key_release(self, key: int, modifiers: List[int]):
        self.active_keys.remove(key)
        self.on_key_release.fire(key, modifiers)

    def _handle_mouse_click(self, x: int, y: int, button: int, modifiers: List[int]):
        self.on_mouse_click.fire(button, x, y, modifiers)

    def _handle_mouse_move(self, x: int, y: int, _dx: int, _dy: int):
